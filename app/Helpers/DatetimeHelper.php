<?php
    ...
    function readableTimeBetweenTwoDates($starDate, $endDate = null) {
        ...
        $readableDiff = "";
        if($diff->y) $readableDiff .= trans_choice('datetime.year', $diff->y, ['value' => $diff->y]);
        if($diff->y && $diff->m) $readableDiff .= trans('datetime.year_month_separator');
        if($diff->m) $readableDiff .= trans_choice('datetime.month', $diff->m, ['value' => $diff->m]);
        return $readableDiff;
    }